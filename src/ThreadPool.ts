...
export class ThreadPool {
    ...
    private enqueue(job: Job<any>) {
        const availableWorker = this.workers.find(worker => !this.workerJobMap.has(worker));
        if (availableWorker) {
    ...
    private onMessage(worker: Worker, message: WorkerMessage) {
        ...
        if (!job) return;
        message.error ? job.reject(new Error(message.error)) : job.resolve(message.result);
