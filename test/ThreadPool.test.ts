...
describe('ThreadPool', () => {
    ...
    test('should execute 10 blockThreadForOneSecond jobs concurrently', async () => {
        ...
        const promises = Array(arrayLen).fill(0).map((_, i) => pool.thread(blockThreadForOneSecond));
    ...
    it('should not accept new jobs while closing', async () => {
        await pool.close();
        await expect(pool.thread(() => { })).rejects.toThrow('Cannot accept new jobs while closing');
    });

    it('should handle errors correctly', async () => {
        await expect(pool.thread(() => { throw new Error('error') })).rejects.toThrow('error');
    });
    ...
    it('should close all workers gracefully after completing ongoing jobs', async () => {
        ...
        const result = pool.thread(() => new Promise(resolve => setTimeout(() => resolve('done'), 100)));
        await pool.close();
